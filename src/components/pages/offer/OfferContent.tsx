...
// Import de l'action Backend (assure-toi d'avoir créé ce fichier)
...
const offerDetails = [
    {
        category: "STRATÉGIE & ARCHITECTURE",
        items: [
            "Analyse de votre marché & besoins",
            "Définition de la stack technique optimale",
    ...
    {
        category: "DÉVELOPPEMENT NATIF",
        items: [
            "Front-end performant (Next.js / React)",
            "Back-end robuste & scalable (NestJS)",
            "Base de données optimisée (Prisma)",
            "Déploiement conteneurisé (Docker)"
        ]
    },
    {
        category: "AUTOMATISATION & I.A.",
        items: [
            "Workflows automatisés (n8n)",
            "Intégration d'outils tiers (CRM, ERP)",
            "Génération de processus IA"
        ]
    },
    {
        category: "PROJECT MANAGEMENT",
        items: [
            "Développeur & Designer dédiés",
    ...
    return (
        <section className="pt-32 pb-24 px-4 md:px-8 max-w-7xl mx-auto min-h-screen">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
                ...
                >
                    <div className="bg-white/5 border border-white/10 rounded-[32px] p-6 md:p-8 flex flex-col shadow-2xl backdrop-blur-md">

                        <button className="w-full bg-nuru-primary text-white font-glitz tracking-widest text-lg py-4 rounded-2xl mb-10 hover:scale-[1.02] transition-transform shadow-[0_0_20px_rgba(230,12,115,0.4)]">
                            Réserver un appel
                        </button>

                        <h1 className="text-3xl font-bold text-white mb-2 font-glitz tracking-wide">
                            L'Offre Nuru
                        </h1>
                        <p className="text-nuru-text/70 text-sm mb-8 font-medium">
                            Une solution clé en main pour propulser votre écosystème digital.
                        </p>

                        <div className="space-y-8">
                            {offerDetails.map((section, idx) => (
                                <div key={idx} className="flex flex-col">
                                    <h3 className="text-[11px] font-bold text-white/50 uppercase tracking-widest mb-4">
                                        {section.category}
                                    </h3>
                                    <ul className="space-y-3">
                                        {section.items.map((item, itemIdx) => (
                                            <li key={itemIdx} className="flex items-start gap-3">
                                                <CheckCircle2 className="w-4 h-4 text-nuru-blue shrink-0 mt-0.5" />
                ...
                {/* === COLONNE DROITE : LE CALENDRIER & SOCIAL PROOF === */}
                <div className="lg:col-span-8 flex flex-col gap-12">

                    {/* --- LE CALENDRIER FULL NEXT.JS --- */}
                    <motion.div
                        initial={{ opacity: 0, y: 30 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ duration: 0.6, delay: 0.2, ease: [0.32, 0.72, 0, 1] }}
                        className="w-full bg-[#0F111A] border border-white/10 rounded-[32px] overflow-hidden shadow-2xl text-white font-sans flex flex-col md:flex-row min-h-[500px]"
                    >
                        {/* INFOS DE L'APPEL (Gauche du Calendrier) */}
                        <div className="w-full md:w-[300px] p-8 border-b md:border-b-0 md:border-r border-white/10 bg-white/5 flex flex-col">
                            <div className="">
                                <span className="font-glitz text-nuru-primary font-bold text-2xl">
                                    <motion.div
                                        initial={{ opacity: 0, y: 20 }}
                                        whileInView={{ opacity: 1, y: 0 }}
                                        viewport={{ once: true }}
                                        className="w-7 h-7 md:w-15 md:h-15 rounded-full overflow-hidden border-2 border-nuru-primary/50 mb-8 relative shadow-[0_0_30px_rgba(230,12,115,0.3)]"
                                    >
                        ...
                        {/* VUES DYNAMIQUES DU CALENDRIER (Droite) */}
                        <div className="flex-1 relative overflow-hidden bg-[#0A0C13]">
                            <AnimatePresence mode="wait">

                                {/* VUE 1 : CALENDRIER ET HEURES */}
                                {step === "calendar" && (
                                    <motion.div
                                        key="calendar"
                                    ...
                                    >
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`
                                                                ...
                                                                ${isPast ? 'text-gray-700 cursor-not-allowed' : 'hover:bg-white/10'}
                                                                ${isSelected ? 'bg-nuru-primary text-white hover:bg-nuru-primary shadow-[0_0_15px_rgba(230,12,115,0.4)]' : ''}
                                                            `}
                                                        >
                                                            {day}
                                                        </button>
                                        ...
                                        {selectedDate && (
                                            <motion.div
                                                ...
                                                className="w-full xl:w-[250px] border-t xl:border-t-0 xl:border-l border-white/10 p-8 bg-[#0F111A]"
                                            >
                                                    ...
                                                    {TIME_SLOTS.map(time => (
                                                        <button
                                                            ...
                                                            className={`
                                                                ...
                                                                ${selectedTime === time
                                                                    ? 'border-nuru-primary bg-nuru-primary/20 text-white'
                                                                    : 'border-white/10 text-gray-300 hover:border-nuru-primary/50 hover:bg-white/5'}
                                                            `}
                                                        >
                                                            {time}
                                                        </button>
                                ...
                                {/* VUE 2 : FORMULAIRE DE CONFIRMATION AVEC BACKEND */}
                                {step === "form" && (
                                    <motion.div
                                        key="form"
                                        initial={{ opacity: 0, x: 20 }}
                                    ...
                                    >
                                        <button
                                            ...
                                            className="flex items-center gap-2 text-sm text-gray-400 hover:text-white transition-colors mb-8 w-max"
                                        >
                                        ...
                                        <form onSubmit={handleSubmit} className="flex flex-col gap-5 flex-1">
                                            ...
                                            <button
                                                type="submit"
                                                disabled={isPending}
                                                className="mt-auto w-full bg-nuru-primary text-white font-bold py-4 rounded-xl hover:scale-[1.02] transition-transform shadow-[0_0_15px_rgba(230,12,115,0.4)] disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
                                            >
                                                {isPending ? (
                                ...
                                {step === "success" && (
                                    ...
                                    >
                                        ...
                                        <button
                                            ...
                                            className="bg-white/10 hover:bg-white/20 border border-white/10 text-white px-6 py-3 rounded-full font-bold transition-colors"
                                        >
                                            Retour à l'accueil
                                        </button>
                                    </motion.div>
                    ...
                    <motion.div
                        ...
                        className="flex flex-wrap items-center justify-center gap-8 md:gap-16 px-4"
                    >
                        {[1, 2, 3, 4].map((item) => (
                            <div key={item} className="opacity-50 hover:opacity-100 transition-opacity duration-300 grayscale hover:grayscale-0">
                                <Image
                                    src="/assets/logo.svg"
                                    alt="Client"
                                    width={120}
                    ...
                    >
                        <p className="text-white text-lg md:text-xl font-medium italic mb-6 max-w-2xl">
                            "Site exceptionnel, nuru agency c'est de la bombe"
                        </p>

                        <div className="flex flex-col md:flex-row items-center gap-6">
                            <div className="flex items-center gap-3">
                                <div className="w-12 h-12 rounded-full overflow-hidden relative border border-white/20">
                                    <Image src="/assets/icon.png" alt="Mohamed Bechir" fill className="object-cover" />
                                </div>
                                <span className="text-white font-bold text-sm">Mohamed bechir</span>
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                                {[1, 2, 3, 4, 5].map((star) => (
                                    <Star key={star} className="w-5 h-5 fill-[#FFD700] text-[#FFD700] drop-shadow-sm" />
                                ))}
                            </div>

                            <div className="hidden md:block w-px h-6 bg-white/20" />

                            <div className="flex items-center gap-2 border border-white/20 rounded-full px-4 py-1.5 bg-white/5">
                                <span className="text-white text-xs font-bold uppercase tracking-wider">Client reviews</span>
                                <div className="bg-nuru-primary text-white text-xs font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                                    <Star className="w-3 h-3 fill-white" /> 4.6
                                </div>
                            </div>
                        </div>
                    </motion.div>
